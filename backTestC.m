classdef backTestC < handle
    % BACKTEST
    %
    % obj = backTest(finData, buyStrategy, sellStrategy)
...
%% Informacoes Principais
        ...
        sellStrat;
        % Function Handle da Estrategia de Entrada
        buyStrat;
...
%% Variaveis de Controle
    ...
    methods
%% Create Method
        function obj = backTestC(finData, buyStrategy, sellStrategy)
            %STATE: Objeto gerador de estados
            obj.state = estadoC(finData);
...
%             obj.N = N;
            ...
            %BUYSTRAT: Function Handle contendo a estrategia de Entrada do
            %Mercado.
            obj.buyStrat = buyStrategy;
...
%% Steps
        ...
        function normStep(obj)
            ...
            % Gera um novo Estado
            obj.curState = getEstado(obj.state,obj.time);
            obj.oldState = getEstado(obj.state,obj.time-1);
            s = [obj.oldState, obj.curState];
            comprado = obj.byslFlag;

            % Verifica a situacao atual da estrategia
            if obj.byslFlag
                % Se estiver Dentro do Mercado, roda a estrategia de Saida
                obj.byslFlag = obj.sellStrat(obj.finPoint,...
                    obj.currentOp, s);
...
%                     obj.transit = 1;
 %                    resetE(obj.rLearnObj);
                ...
                obj.byslFlag = obj.buyStrat(obj.finPoint,...
                    obj.currentOp, s);
...
%% Operations
        ...
        function reward = sellLong(obj)
            % Efetua a Venda
            reward = sell(obj.currentOp, obj.finPoint, obj.time, obj.finData);
            % Registra a compra
...
%% Plots
    ...
    function [evolperc, evol2] = moneyEvol(obj)
        ...
        nome = [papel '/' obj.finData.gran 'MonEvoC.png'];
        saveas(1, nome, 'png');
        ...
        nome = [papel '/' obj.finData.gran 'MonPerC.png'];
        saveas(2, nome, 'png');
        ...
        nome = [papel '/' obj.finData.gran 'OcorrC.png'];
        saveas(3, nome, 'png');
        close all;
    end
